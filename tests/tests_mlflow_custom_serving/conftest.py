"""
This module contains the configuration and fixtures for the project services

Session and test running activities will invoke all hooks defined in conftest.py files closer
...
from tests_mlflow_custom_serving.conf.tests_data_conf import TestsDataConf
...
# Environment paths
TESTS_SRC_PATH: Path = Path(__file__).parent
TESTS_PATH: Path = TESTS_SRC_PATH.parent
TESTS_ENV_PATH = TESTS_PATH.joinpath('.env.test')

ROOT_PATH: Path = TESTS_PATH.parent

# data directory manager
data_mgr: TestsDataConf = TestsDataConf(_env_file=str(TESTS_ENV_PATH), data_path=TESTS_PATH.joinpath('data'))

# Packages
TARGET_TESTS: str = "tests_mlflow_custom_serving"
...
def pytest_deselected(items):
    """
    If you want a report in another format, simply store the deselected items in the config
    and use them for the desired output somewhere else, e.g. pytest_terminal_summary
    """
    if not items:
        return
    config = items[0].session.config
